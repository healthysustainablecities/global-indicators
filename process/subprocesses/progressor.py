"""
A progress percent timer function.
...
def progressor(num=0, denom=100, start=None, task=''):
    ...
    if num >= 0:
        ...
        if type(start) in (int, float):
            ...
            eta = ' (ETA: {}) '.format(
                time.strftime('%Y%m%d_%H%M', time.localtime(etaT)),
            )
            mult += len(HMS) + len(eta)
        if num == 0:
            todayhour = time.strftime('%Y%m%d_%H%M')
            print(f'Start: {todayhour} ')
            print(
                '{:5.2f}% {}{}'.format(0, task, ' ' * (mult - len(task) - 9)),
            ),
        print('\b' * mult),
        print(f'{pct:5.2f}%{HMS}{eta} {task}'),
        if num >= denom:
            todayhour = time.strftime('%Y%m%d_%H%M')
            print(f'\nComplete: {todayhour}')
            if num > denom:
...
if __name__ == '__main__':
  ...
  """,
    ...
    for num in range(0, denom + 1):
        progressor(num, denom)
        time.sleep(1)
    print(f'Task duration = {(time.time() - start) / 60:9.2f}')
  ...
  """,
    ...
    for num in range(0, denom + 1):
        progressor(num, denom, start, taskName)
        time.sleep(1)
    print(f'Task duration = {(time.time() - start) / 60:9.2f} minutes')
