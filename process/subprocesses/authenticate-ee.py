...
def earth_engine_auth():
    ...
    if not os.path.exists(adc_path):
        ...
        print(
            "If you haven't already, please register a Google Earth Engine account and setup a corresponding Google Cloud Project via the following link:",
        )
        print('https://earthengine.google.com/noncommercial/')
        print(
            'During the registration process, please note your unique project ID and enter it below.',
        )

        # Prompt user to input their project ID
        project_id = input(
            '\nPlease enter your unique Google Cloud project ID: ',
        ).strip()
        ...
        print(
            'If you wish to re-authenticate with a different account, run the following command in a seperate terminal whilst the container is running to delete the existing credentials:',
        )
        print(
