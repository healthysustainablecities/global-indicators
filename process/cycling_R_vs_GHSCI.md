...
> **Provenance / authority of this document.** This comparison follows the **current code**
...
> indicator families (a strict fully-low-stress `safe` measure and a stress-penalty `access` measure) with
> uncapped footway dismount at `dismount_weight: 3.0`.
>
> *Update (2026-08-19):* [`subprocesses/cycling_indicators_readme.md`](subprocesses/cycling_indicators_readme.md)
> has since been corrected and now describes the current measure architecture, costs and defaults; it no
> longer lags the code. This document is a **standalone historical comparison** — it is not regenerated per
> run, and the R comparison was removed from the per-city validation report. Where it and the code disagree,
> the code wins. Note also that the user-facing term for the `danger_weighted` measure is now
> **"stress penalty"**; the measure key, column infix and `danger_weight` constant are unchanged.
