...
def main():
    ...
    if not (engine.has_table('edges') and engine.has_table('nodes') and engine.has_table(intersections_table)):
        ...
        print("\nIt appears that edges, nodes and clean intersection data have already been prepared for this region.")

    for feature in ['edges','nodes']:
        print(f"Copy the pedestrian network {feature} from shapefiles to Postgis..."),
        command = (
