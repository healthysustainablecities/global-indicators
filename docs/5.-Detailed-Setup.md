...
- [Custom data](https://healthysustainablecities.github.io/global-indicators/Custom-data) — `points_of_interest`, `public_open_space` and `sampling`, for supplying your own destinations, open space and sample points
...
A completed configuration is provided for the example study region, with data included so that it can be run as-is: [`process/configuration/regions/example_ES_Las_Palmas_2023.yml`](https://github.com/healthysustainablecities/global-indicators/blob/main/process/configuration/regions/example_ES_Las_Palmas_2023.yml).  Copying this file and modifying the values is often the easiest way to start configuring a new region.

An Earth Engine variant is also provided as [`example_ES_Las_Palmas_2023-ee.yml`](https://github.com/healthysustainablecities/global-indicators/blob/main/process/configuration/regions/example_ES_Las_Palmas_2023-ee.yml), which differs only in setting `gee: true` and using the `spatial_ee` and `policy_spatial_ee` report templates.
