...
|Optional      | [region_boundaries](https://healthysustainablecities.github.io/global-indicators/Custom-aggregation)  | Additional boundaries to summarise indicator results for, configured under `custom_aggregations` (e.g. administrative areas, neighbourhoods, catchments) |
|Optional      | [other_custom_data](https://healthysustainablecities.github.io/global-indicators/Custom-data)  | Your own destinations (`points_of_interest`), public open space (`public_open_space`), or sample point locations (`sampling`), supplementing or replacing what is derived from OpenStreetMap |
...
Destinations, public open space and sample point locations may also be supplied from your own data, to supplement or replace what is otherwise derived from OpenStreetMap.  These are configured under `points_of_interest`, `public_open_space` and `sampling` respectively; see [Custom data](https://healthysustainablecities.github.io/global-indicators/Custom-data).

Such data is usually stored in the `process/data/other_custom_data` folder, although any path relative to `process/data` may be configured.
