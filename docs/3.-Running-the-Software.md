...
## Optional: the Earth Engine container

Two optional indicators — availability of and access to large public urban green space, and the Global Urban Heat Vulnerability Index — are generated using Google Earth Engine, and require a different container to the standard one.  To use them, launch with `.\global-indicators-ee.bat` on Windows, or `bash ./global-indicators-ee.sh` on MacOS and Linux, in place of the usual launcher.  Everything else works the same way.

This additionally requires a Google Cloud project registered for Earth Engine, and `gee: true` in your study region configuration.  See [Earth Engine indicators](https://healthysustainablecities.github.io/global-indicators/Earth-Engine-indicators) for what is involved and what is produced.
