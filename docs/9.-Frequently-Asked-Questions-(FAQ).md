...
The tool can also be used to summarise and visualise policy indicators data collected using the [1000 Cities challenge policy checklist tool](https://github.com/healthysustainablecities/global-indicators/raw/refs/heads/main/process/data/policy_review/gohsc-policy-indicator-checklist.xlsx).  A policy-only report can alternatively be generated online using [GHSCI Policy](https://policy.healthysustainablecities.org/), without installing this software; see [policy indicators](https://healthysustainablecities.github.io/global-indicators/1.-Policy-Indicators).
...
A Policy Indicators Report can be generated online using [GHSCI Policy](https://policy.healthysustainablecities.org/) — sign in and either upload your completed checklist or complete the equivalent online form.  A Policy Indicators Report or combined Policy and Spatial Indicators Report (if you have also completed spatial indicators) can also be generated through the GHSCI software you install and run yourself.  The two pathways are compared under [policy indicators](https://healthysustainablecities.github.io/global-indicators/1.-Policy-Indicators#choosing-your-pathway).  Please contact the 1000 Cities Challenge team at info@healthysustainablecities.org for support using either tool, or we can assist with generating Reports for you.

:small_blue_diamond: **Which version of the software should I use to generate a report that includes the policy indicators?**
Policy checklist version v2.\* of the checklist was [released](https://github.com/healthysustainablecities/global-indicators/releases/tag/v4.10.0) on 30 October 2025.  For users with policy checklists marked v1.\* it is recommended to either use the older software version [v4.9.1](https://github.com/healthysustainablecities/global-indicators/releases/tag/v4.9.1) to support policy reporting, or migrate to using v2.\*. We do not currently have an automated migration tool, however if users indicate this is something that would be valued that is something we could consider developing.  Users of policy checklists marked v2.\* can use the [latest software release](https://github.com/healthysustainablecities/global-indicators/releases) (v4.10.0 and higher), or [GHSCI Policy](https://policy.healthysustainablecities.org/) online, which accepts v2.\* checklists.
...
For larger study regions, and/or to make the most of more powerful computers, it may be necessary the resources allocated to the Docker Desktop engine that runs the GHSCI software.  How this is done depends on the computer system you are using, as described in Docker Desktop's manual [here](https://docs.docker.com/desktop/settings-and-maintenance/settings/#resources). On Windows computers using Windows Subsystem for Linux, this can be defined in a .wslconfig text file that you can create and locate in your user directory.  For example, as described by this user conducting an analysis with GHSCI on a Microsoft Surface laptop PC with 16GB of RAM and 8 processors, they configured their analysis to run with the following settings:
...
Here is an example of a file for a desktop computer, with some addition guidance in comments:
```
[wsl2]
memory=24GB
...
If you running a Windows computer using WSL2, you can use the above as a template, modifying the values to reflect the resources available for your system.

- **Insufficient Memory**
Running large study areas can require substantial memory. For example, a particular analysis configured for the Los Angeles Metropolitan area (with a large and complex street network) required reducing the chunk size of analyses to run the analysis. The `chunk_size` parameter can be found in the `config.yml` [file](https://github.com/healthysustainablecities/global-indicators/blob/main/process/configuration/templates/config.yml) in the `process/configuration` directory, and contains comments that can guide its modification for your needs.    
- **System and Docker Updates**      
Make sure your operating system and Docker Desktop are fully updated before beginning the analysis. To download the latest version of Docker Desktop, visit the [official site](https://www.docker.com/products/docker-desktop/) or check updates in your existing software.       
- **Anticipate Time Consumption**    
Neighbourhood analysis (step 11) takes longer for larger study regions. Consider running smaller test areas first.  It is possible that analysis for larger study regions could take a number of hours to complete.     

- **Database issues when updating to v4.10 of the software**
Switching to version v4.10.* or higher after using an older version of the software requires removal of the pre-existing `ghsci_postgis` container in Docker Desktop.  Doing this will require repeating any pre-existing analyses, if present.  This is because for the new version we have updated the postgresql database image used and database location. A symptom of this problem is a failure to connect to the database when conducting analysis, recorded in the log text file in the study region output directory.
